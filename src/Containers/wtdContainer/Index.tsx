...
const IndexWTDContainer = () => {
  ...
  const selectActivity = (id: number, type : string) => navigate('wtdDetail', {id:id,type:type})

  const alldata = useSelector((state: { alldata: AllDataState }) => state.alldata.item.data)

  const filter = alldata.wtdcategories.filter( (object : any) => object['what_to_dos'].length > 0)
